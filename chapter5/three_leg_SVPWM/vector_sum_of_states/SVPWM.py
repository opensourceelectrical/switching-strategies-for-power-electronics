...
# In this file, only switching combinations for each vector are stored.
# Which vector comes first in the sequence is the one with least ON state devices
...
if t_clock > t1:  # t_clock is an internal variable
    ...
    # Vector Selection
    if (sumVec(V[sec - 1]) < sumVec(V[sec % 6])):
        index_x = sec - 1
        index_y = sec % 6
    else:
        index_x = sec % 6
